...
- [ ] 确认服务器系统：不建议使用 Windows Server 2012，优先选择 Linux 发行版。
- [ ] 确认生产部署资源：2 核 2G 可用于早期测试，小规模上线建议至少 2 核 4G，并将 PostgreSQL / Redis 托管或单独部署。
...
- [!] 微信登录如何与现有 Better Auth 用户体系迁移或替换需要设计。
- [!] AIGC 内容和世界聊天需要内容安全策略。
- [!] 小程序包体限制可能要求分包。
- [!] React + Tailwind UI 无法直接复用，需要大量重写。
- [!] 后端已有服务器，但仍需确认 HTTPS 域名和微信合法请求域名配置。
- [!] 截图中的 2 核 2G 4M 服务器可做开发/内测，但 Windows Server 2012 不适合作为长期生产环境；优先改用 Linux。
